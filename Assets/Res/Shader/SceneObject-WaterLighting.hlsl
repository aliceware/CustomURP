...
half TotalInternalReflection(half3 normalWS, half3 viewDirectionWS){
    // half min = lerp(0.2, 1, viewDirectionWS.y);
    half ndotv = saturate(dot(normalWS, viewDirectionWS));
    // half TIR = smoothstep(min, 1, ndotv);
    half TIR = smoothstep(0.5, 1, ndotv);
    return TIR;
}
...
//         Light mainLight = GetMainLight(TransformWorldToShadowCoord(lightJitter));
